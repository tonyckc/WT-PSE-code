...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        img, mask_od, mask_oc  = sample['image'], sample['label_od'], sample['label_oc']
        # print(img.size)
        w, h = img.size
        if self.padding > 0 or w < self.size[0] or h < self.size[1]:
            ...
            mask_oc = ImageOps.expand(mask_oc, border=padding, fill=255)
            mask_od = ImageOps.expand(mask_od, border=padding, fill=255)


        assert img.width == mask_oc.width
        assert img.height == mask_oc.height
        w, h = img.size
        ...
        mask_od = mask_od.crop((x1, y1, x1 + tw, y1 + th))
        mask_oc = mask_oc.crop((x1, y1, x1 + tw, y1 + th))
        # print(img.size)
        sample['image'] = img
        sample['label_od'] = mask_od
        sample['label_oc'] = mask_oc
        return sample
...
class RandomScaleCrop(object):
    ...
    def __call__(self, sample):
        img = sample['image']
        mask_od = sample['label_od']
        mask_oc = sample['label_oc']
        # print(img.size)
        assert img.width == mask_od.width
        assert img.height == mask_od.height
        ...
        if seed > 0.5:
            ...
            img, mask_od, mask_oc = img.resize((w, h), Image.BILINEAR), mask_od.resize((w, h), Image.NEAREST), mask_oc.resize((w, h), Image.NEAREST)
            sample['image'] = img
            sample['label_od'] = mask_od
            sample['label_oc'] = mask_oc
...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        img = img.resize((self.size, self.size))
        mask_od = mask_od.resize((self.size, self.size))
        mask_oc = mask_oc.resize((self.size, self.size))
        sample = {'image': img, 'label_od': mask_od, 'label_oc': mask_oc,'dc': dc}
        return sample
...
class Normalize_tf(object):
    ...
    def __call__(self, sample):
        ...
        img = np.array(sample['image']).astype(np.float32)
        __mask_od = np.array(sample['label_od']).astype(np.uint8)
        __mask_oc = np.array(sample['label_oc']).astype(np.uint8)
        img /= 127.5
        img -= 1.0
        _mask_od = np.zeros([__mask_od.shape[0], __mask_od.shape[1]])
        _mask_od[__mask_od > 200] = 255
        # index = np.where(__mask > 50 and __mask < 201)
        _mask_od[(__mask_od > 50) & (__mask_od < 201)] = 128
        _mask_od[(__mask_od > 50) & (__mask_od < 201)] = 128


        __mask_od[_mask_od < 255] = 1
        __mask_od[_mask_od == 255] = 0

        mask_od = np.expand_dims(__mask_od, axis=2)  # to_multilabel(__mask)
        sample['image'] = img
        sample['label_od'] = mask_od

        _mask_oc = np.zeros([__mask_oc.shape[0], __mask_oc.shape[1]])
        _mask_oc[__mask_oc > 200] = 255
        # index = np.where(__mask > 50 and __mask < 201)
        _mask_oc[(__mask_oc > 50) & (__mask_oc < 201)] = 128
        _mask_oc[(__mask_oc > 50) & (__mask_oc < 201)] = 128

        __mask_oc[_mask_od > 0] = 0
        __mask_oc[_mask_od == 0] = 1

        mask_oc = np.expand_dims(__mask_oc, axis=2)  # to_multilabel(__mask)
        sample['label_oc'] = mask_oc

        return sample

class Normalize_single_label(object):
    """Normalize a tensor image with mean and standard deviation.
    Args:
        mean (tuple): means for each channel.
        std (tuple): standard deviations for each channel.
    """
    def __init__(self, label=None,mean=(0., 0., 0.), std=(1., 1., 1.)):
        self.mean = mean
        self.std = std
        self.label = label
        self.get_boundary = GetBoundary()
    ...
    def __call__(self, sample):
        ...
        mask = np.expand_dims(__mask,axis=2) #to_multilabel(__mask)
        sample['image'] = img
        sample['label'] = mask
        return sample
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        map_od = np.array(sample['label_od']).astype(np.uint8).transpose((2, 0, 1))
        map_oc = np.array(sample['label_oc']).astype(np.uint8).transpose((2, 0, 1))
        img = torch.from_numpy(img).float()
        map_od = torch.from_numpy(map_od).float()
        map_oc = torch.from_numpy(map_oc).float()
        sample['image']=img
        sample['label_od']=map_od
        sample['label_oc'] = map_oc
